...
impl Interface {
    pub async fn get_game_ids(&self, start_timestamp: &String, puuid: &String) -> Result<Vec<String>, Box<dyn Error>> {
        ...
        let resp = reqwest::get(
            format!(
                "https://{}.api.riotgames.com/lol/match/v5/matches/by-puuid/{}&api_key={}",
                self.server, puuid, start_timestamp, self.api_key)
            ).await.unwrap().text().await.unwrap();
        let mut deserialize = Deserializer::from_str(&resp);
    ...
    pub async fn get_match_data_collection(&self, ids: Vec<String>, puuid: &String) -> Result<Vec<RawData>, Box<dyn Error>> {
        ...
        for id in &ids {
            ...
            if check_valid_game(&game) {
                ...
                let raw = RawData::new(game_data, game_tl);
                raw.find_me(puuid);
                out.push(raw);
            }
        }
    ...
    async fn request_game_data(&self, id: &String) -> Result<MatchData, Box<dyn Error>> {
        let resp = reqwest::get(
            format!("https://{}.api.riotgames.com/lol/match/v5/matches/{}?api_key={}", id, self.api_key)
        ).await
            .unwrap()
            .text()
            .await
    ...
    async fn request_match_timeline(&self, id: &String) -> Result<Timeline, Box<dyn Error>> {
        let resp = reqwest::get(format!("https://americas.api.riotgames.com/lol/match/v5/matches/{}/timeline?api_key={}", id, self.api_key)).await.unwrap().text().await.unwrap();
